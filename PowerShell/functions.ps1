...
# Initializes Conda in the current session on demand without profile startup overhead.
function Initialize-Conda {
    [CmdletBinding()]
    param(
        [string]$CondaExePath = "$env:USERPROFILE\miniconda3\Scripts\conda.exe",
        [switch]$Quiet
    )

    if ($script:CondaInitialized) {
        if (-not $Quiet) {
            Write-Host 'Conda is already initialized for this session.' -ForegroundColor DarkGray
        }
        return $true
    }

    if (-not (Test-Path -Path $CondaExePath)) {
        Write-Warning "Conda executable not found at '$CondaExePath'."
        Write-Warning 'Install Conda or pass -CondaExePath with the correct location.'
        return $false
    }

    $existingPrompt = $null
    $hasPromptFunction = Test-Path -Path Function:\prompt
    if ($hasPromptFunction) {
        $existingPrompt = (Get-Item -Path Function:\prompt).ScriptBlock
    }

    $previousCondaChangePs1 = $env:CONDA_CHANGEPS1
    $env:CONDA_CHANGEPS1 = 'false'

    try {
        $condaHook = & $CondaExePath 'shell.powershell' 'hook' | Out-String

        if ([string]::IsNullOrWhiteSpace($condaHook)) {
            throw 'Conda hook output was empty.'
        }

        Invoke-Expression $condaHook

        if ($hasPromptFunction -and $null -ne $existingPrompt) {
            Set-Item -Path Function:\prompt -Value $existingPrompt
        }

        $script:CondaInitialized = $true

        if (-not $Quiet) {
            Write-Host 'Conda initialized. You can now run conda activate <env>.' -ForegroundColor Green
        }
        ...
        if ($null -eq $previousCondaChangePs1) {
            Remove-Item -Path Env:\CONDA_CHANGEPS1 -ErrorAction SilentlyContinue
        }
        else {
            $env:CONDA_CHANGEPS1 = $previousCondaChangePs1
        }
    }
}
